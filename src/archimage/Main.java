...
public class Main extends mangFu.Main
{
    boolean havek=false;
    private long time=0;
    ...
    public ArrayList<String> operate(Info now) {
        ...
        if((s=naturalReact(now)).size()!=0) last=s;
        else if((s=attack(now)).size()!=0) last=s;
        else {
            if(!now.A.isrotate) s.add("rotate");
            else if(rand.nextDouble()<0.5*state+0.5*situation) s=move(now,false);
            else s=move(now,true);
        }
